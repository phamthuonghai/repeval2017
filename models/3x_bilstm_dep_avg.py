...
class MyModel(object):
    def __init__(self, **kwargs):
        ...
        self.W_mlp2 = tf.Variable(tf.random_normal([self.dim, self.dim], stddev=0.1))
        self.b_mlp2 = tf.Variable(tf.random_normal([self.dim], stddev=0.1))
        ...
        # MLP layer 2
        h_mlp2 = tf.nn.relu(tf.matmul(h_mlp, self.W_mlp2) + self.b_mlp2)
        # Dropout applied to classifier 2
        h_drop2 = tf.nn.dropout(h_mlp2, self.keep_rate_ph)

        # Get prediction
        self.logits = tf.matmul(h_drop2, self.W_cl) + self.b_cl
